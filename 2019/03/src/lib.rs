...
impl Path {
    ...
    fn intersections(&self, other: &Self) -> HashSet<Point> {
        ...
        for point in &self.0 {
            if let Some(other) = other.0.get(&point) {
                intersections.insert(Point {
                    x: point.x,
                    y: point.y,
                    cost: point.cost + other.cost,
                });
            }
        }
...
pub fn part2() {
    ...
    let cheapest = intersections.iter()
        .min_by_key(|p| p.cost)
        .unwrap();

    println!("{}", cheapest.cost);
}
...
mod test {
    ...
    const CASE_0_WIRE_1: &'static str = "R8,U5,L5,D3";
    const CASE_0_WIRE_2: &'static str = "U7,R6,D4,L4";

    const CASE_1_WIRE_1: &'static str = "R75,D30,R83,U83,L12,D49,R71,U7,L72";
    const CASE_1_WIRE_2: &'static str = "U62,R66,U55,R34,D71,R55,D58,R83";

    const CASE_2_WIRE_1: &'static str = "R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51";
    const CASE_2_WIRE_2: &'static str = "U98,R91,D20,R16,D67,R40,U7,R15,U6,R7";

    #[test]
    fn part1_case0() {

        let wire_1: Path = CASE_0_WIRE_1.parse()
            .unwrap();
        let wire_2: Path = CASE_0_WIRE_2.parse()
            .unwrap();
        let intersections = wire_1.intersections(&wire_2);
        ...
        assert_eq!(nearest.man_dist(), 6);
    }

    #[test]
    fn part1_case1() {

        let wire_1: Path = CASE_1_WIRE_1.parse()
            .unwrap();
        let wire_2: Path = CASE_1_WIRE_2.parse()
            .unwrap();
        let intersections = wire_1.intersections(&wire_2);
        ...
        assert_eq!(nearest.man_dist(), 159);
    }

    #[test]
    fn part1_case2() {

        let wire_1: Path = CASE_2_WIRE_1.parse()
            .unwrap();
        let wire_2: Path = CASE_2_WIRE_2.parse()
            .unwrap();
        let intersections = wire_1.intersections(&wire_2);
        ...
        assert_eq!(nearest.man_dist(), 135);
    }
