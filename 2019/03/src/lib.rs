...
impl FromStr for Move {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        let dir = match &s[..1] {
            ...
            "L" => Direction::Left,
            "R" => Direction::Right,
            _ => return Err(ParseMoveError::Direction),
        };

        let dist: isize = s[1..].parse()?;

        Ok(Self { dir, dist })
    }
}

fn parse_moves(s: &str) -> Result<Vec<Move>, ParseMoveError> {
    s.split(",")
        .map(|m| m.parse::<Move>())
...
impl Point {
    ...
    fn man_dist(&self, other: &Point) -> isize {
...
fn build_path(moves: &[Move]) -> HashSet<Point> {
    ...
    for move_ in moves {
        ...
        for _ in 0..move_.dist {

            x += dx;
            y += dy;
            cost += 1;

            let point = Point(x, y, cost);
...
fn get_intersections(first: &[Move], second: &[Move]) -> HashSet<Point> {

    let first = build_path(first);

    let second = build_path(second);

    let mut intersections = HashSet::new();
    for point in first {
        if let Some(other) = second.get(&point) {
            intersections.insert(Point(point.0, point.1, point.2 + other.2));
        }
    }
...
fn closest_intersection(first: &[Move], second: &[Move]) -> Point {

    let intersections = get_intersections(first, second);
    let origin = Point(0, 0, 0);

    intersections.into_iter()
        .min_by_key(|p| origin.man_dist(p))
        .unwrap()
}

fn cheapest_intersection(first: &[Move], second: &[Move]) -> Point {
